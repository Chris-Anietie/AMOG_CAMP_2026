...
// CONSTANTS
const GRACE_SCHOOLS = ['Group 1', 'Group 2', 'Group 3', 'Group 4', 'Group 5', 'Group 6']; // <--- FIXED: Added this back
...
function RegistrationModal({ isOpen, onClose, onRegister, processing }: any) {
  const [data, setData] = useState({
    full_name: '', phone_number: '', role: 'Member', branch: '', gender: 'Male',
      ...
      <div className="bg-[#1e293b] w-full max-w-2xl rounded-3xl border border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="group">
                ...
                <div className="relative"><User className="absolute left-3 top-3 w-4 h-4 text-slate-500 group-focus-within:text-indigo-400 transition-colors" />
                  <input type="text" className="w-full bg-slate-900/50 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-white focus:ring-2 focus:ring-indigo-500/50 outline-none transition-all placeholder:text-slate-600" placeholder="John Doe" value={data.full_name} onChange={e => setData({ ...data, full_name: e.target.value })} />
                </div>
              </div>
              <div className="group">
                <label className="text-xs font-medium text-slate-300 mb-1.5 block">Phone Number</label>
                <div className="relative"><Smartphone className="absolute left-3 top-3 w-4 h-4 text-slate-500 group-focus-within:text-indigo-400 transition-colors" />
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select className="w-full bg-slate-900/50 border border-white/10 rounded-xl py-2.5 px-4 text-white focus:ring-2 focus:ring-indigo-500/50 outline-none appearance-none" value={data.branch} onChange={e => setData({ ...data, branch: e.target.value })}>
                  <option value="">Select Branch...</option>{CHURCH_BRANCHES.map(b => <option key={b} value={b} className="bg-slate-900">{b}</option>)}
                </select>
              </div>
              <div>
                ...
                <div className="flex bg-slate-900/50 p-1 rounded-xl border border-white/10">
                  {['Male', 'Female'].map(g => (
                    <button key={g} type="button" onClick={() => setData({ ...data, gender: g })} className={`flex-1 text-xs font-bold py-2 rounded-lg transition-all ${data.gender === g ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>{g}</button>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div>
                ...
                <div className="flex bg-slate-900/50 p-1 rounded-xl border border-white/10">
                  {['Member', 'Leader', 'Pastor', 'Guest'].map(r => (
                    <button key={r} type="button" onClick={() => setData({ ...data, role: r })} className={`flex-1 text-xs font-bold py-2 rounded-lg transition-all ${data.role === r ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}>{r}</button>
                  ))}
                </div>
            </div>
          </div>
          ...
          <div className="bg-indigo-900/10 border border-indigo-500/20 rounded-2xl p-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-sm font-bold text-indigo-200 flex items-center gap-2">Camp T-Shirt <span className="text-[10px] bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded border border-indigo-500/30">OPTIONAL</span></h3>
            ...
            {data.wants_tshirt && (
              <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2 fade-in">
                <div>
                  ...
                  <select className="w-full bg-slate-900/50 border border-indigo-500/30 rounded-xl py-2 px-3 text-white outline-none" value={data.t_shirt} onChange={e => setData({ ...data, t_shirt: e.target.value })}>
                ...
                <div>
                  ...
                  <input type="text" className="w-full bg-slate-900/50 border border-indigo-500/30 rounded-xl py-2 px-3 text-white outline-none placeholder:text-slate-600" placeholder="e.g. White" value={data.t_shirt_color} onChange={e => setData({ ...data, t_shirt_color: e.target.value })} />
                </div>
              </div>
            )}
          </div>
        ...
        <div className="p-6 border-t border-white/10 bg-slate-900/50 flex gap-3">
          ...
          <button onClick={() => onRegister(data)} disabled={processing} className="flex-[2] py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl shadow-lg shadow-indigo-900/20 transition-all flex items-center justify-center gap-2">
            {processing ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Plus className="w-5 h-5" /> Register Camper</>}
          </button>
        </div>
      </div>
    </ModalBackdrop>
                ...
                <div className="p-6 bg-gradient-to-r from-slate-900 to-indigo-950/50 border-b border-white/10 flex justify-between items-start">
                    <div className="flex gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-indigo-400 font-mono mt-1">RCPT #: {person.receipt_no || '---'}</p>
                        </div>
                    </div>
...
export default function Home() {
  ...
  const [momoTransId, setMomoTransId] = useState(''); // NEW: Track MoMo ID
  ...
  async function fetchPeople() {
     ...
     // UPDATE: Only fetch users where is_deleted is FALSE
  ...
  async function initiateDelete(person: any) {
    ...
    const { error } = await supabase.from('participants')
        .update({ is_deleted: true }) // We update, we DO NOT delete
        .eq('id', person.id);

    if(error) showToast("Delete failed", "error");
    else showToast("User removed from view", "success");
  }
  ...
  async function handlePayment() {
      ...
      if(paymentMethod === 'MoMo') {
        updateData.momo_transaction_id = momoTransId; // Save the ID
      }
      ...
      if(error) showToast("Payment failed", "error");
          ...
          await logAction('Payment Received', `Payment for ${selectedPerson.full_name}: Recorded ₵${amount} via ${paymentMethod}. MoMoID: ${momoTransId || 'N/A'}`);
          showToast("Payment Recorded", "success");
          setSelectedPerson(null);
          setMomoTransId(''); // Reset
      }
      setProcessing(false);
  }
  ...
  async function handleAdmit() {
      ...
      // NEW: Gender-Based Logic
      let targetGroups = GRACE_SCHOOLS; // Default fallback
  ...
  if (!session) return (
    <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-6 relative overflow-hidden font-sans">
       ...
       <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl w-full max-w-sm shadow-2xl relative z-10 animate-in fade-in zoom-in-95 duration-500">
           <div className="text-center mb-8"><h1 className="text-4xl font-black text-white tracking-tighter">AMOG <span className="text-indigo-500">2026</span></h1><p className="text-indigo-200 text-xs font-bold uppercase tracking-widest mt-2">Camp Staff Portal</p></div>
           <form onSubmit={async (e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); const { error } = await supabase.auth.signInWithPassword({ email: fd.get('email') as string, password: fd.get('password') as string }); if(error) showToast('Invalid Credentials', 'error'); }} className="space-y-4">
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-100 font-sans pb-20 relative overflow-x-hidden">
        <div className="fixed inset-0 pointer-events-none z-0">
             <img src="/camp-bg.png" onError={(e) => e.currentTarget.src = "https://images.unsplash.com/photo-1478131143081-80f7f84ca84d?q=80&w=2070&auto=format&fit=crop"} className="absolute inset-0 w-full h-full object-cover opacity-20" alt="Camp Background" />
        ...
        <main className="relative z-10 max-w-7xl mx-auto px-4 pt-6 space-y-6">
            ...
            <div className="flex flex-col md:flex-row gap-4">
                <div className="flex-1 relative group">
                    ...
                    <input type="text" placeholder="Search campers..." value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white/5 hover:bg-white/10 focus:bg-black/40 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-white outline-none focus:border-indigo-500/50 transition-all"/>
                </div>
                <div className="flex gap-2 overflow-x-auto pb-1">
                     ...
                     <select value={branchFilter} onChange={e => setBranchFilter(e.target.value)} className="bg-white/5 border border-white/10 text-slate-300 rounded-2xl px-4 py-3 outline-none focus:border-indigo-500 appearance-none">
                        <option value="">All Branches</option>{CHURCH_BRANCHES.map(b => <option key={b} className="bg-slate-900">{b}</option>)}
                     </select>
                </div>
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                {['all', 'owing', 'paid', 'checked_in'].map(f => (
                    <button type="button" key={f} onClick={() => setFilter(f)} className={`px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wider border transition-all whitespace-nowrap ${filter === f ? 'bg-white text-slate-900 border-white' : 'bg-transparent text-slate-400 border-slate-700 hover:border-slate-500'}`}>{f.replace('_', ' ')}</button>
                ))}
            </div>
                    ...
                    return (
                        <div key={p.id} onClick={() => setReportPerson(p)} className="bg-white/5 backdrop-blur-md border border-white/5 rounded-3xl p-5 hover:bg-white/10 hover:border-white/10 transition-all cursor-pointer group active:scale-[0.98]">
                            <div className="flex justify-between items-start mb-4">
                                <div><h3 className="font-bold text-lg text-white leading-tight">{p.full_name}</h3><p className="text-xs text-slate-400 mt-1">{p.branch} • {p.role}</p></div>
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isCheckedIn ? 'bg-indigo-500/20 text-indigo-400' : (isOwing ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400')}`}>{isCheckedIn ? <CheckCircle className="w-4 h-4"/> : (isOwing ? <AlertCircle className="w-4 h-4"/> : <CheckCircle className="w-4 h-4"/>)}</div>
                            </div>
                            
                            <div className="mb-4">
                                {isCheckedIn ? (
                                    <div className="bg-indigo-900/20 border border-indigo-500/20 rounded-xl p-3 text-center"><p className="text-[10px] uppercase text-indigo-300 font-bold tracking-widest">Admitted To</p><p className="text-xl font-bold text-white">{p.grace_school}</p></div>
                                ) : (
                                    <div className="flex items-baseline gap-1">
                                        <span className="text-2xl font-bold text-white font-mono">₵{p.amount_paid}</span><span className="text-xs text-slate-500 font-medium">/ ₵{REG_FEE}</span>
                                ...
                                <p className="text-[9px] text-slate-600 font-mono mt-2 text-right">RCPT-{p.receipt_no}</p>
                            </div>

                            <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                                <button type="button" onClick={() => { setSelectedPerson(p); setModalMode('payment'); }} className="flex-1 bg-white/5 hover:bg-white/10 text-slate-200 py-2.5 rounded-xl text-xs font-bold border border-white/5 transition-colors flex items-center justify-center gap-2"><Coins className="w-3 h-3"/> Pay</button>
                                {!isCheckedIn && (
                                    <button type="button" onClick={() => { setSelectedPerson(p); setModalMode('checkin'); }} disabled={isOwing} className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-2 ${isOwing ? 'bg-white/5 text-slate-600 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-500 text-white'}`}><LogOut className="w-3 h-3 rotate-180"/> Admit</button>
                                )}
                                {/* NEW: Soft Delete Button */}
            ...
            <ModalBackdrop onClose={() => setSelectedPerson(null)}>
                <div className="bg-[#1e293b] w-full max-w-sm rounded-3xl border border-white/10 p-6 animate-in zoom-in-95">
                    ...
                    <div className="bg-white/5 rounded-xl p-4 mb-4">
                        <p className="text-xs text-slate-400">Camper</p><p className="text-white font-bold">{selectedPerson.full_name}</p>
                        <p className="text-xs text-slate-400 mt-2">Current Status</p><p className={`font-mono ${selectedPerson.amount_paid >= REG_FEE ? 'text-emerald-400' : 'text-amber-400'}`}>₵{selectedPerson.amount_paid} Paid</p>
                    </div>
