...
export default function Home() {
  ...
  // MODAL TABS STATE
  const [activeTab, setActiveTab] = useState<'payment' | 'checkin'>('payment');
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in zoom-in-95">
          <div className="bg-[#0f172a] border border-white/10 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="flex border-b border-white/10">
                <button onClick={() => setActiveTab('payment')} className={`flex-1 py-4 text-sm font-bold uppercase tracking-widest transition-colors ${activeTab === 'payment' ? 'bg-white/10 text-white border-b-2 border-indigo-500' : 'text-slate-500 hover:text-slate-300'}`}>💳 Payment</button>
                <button onClick={() => setActiveTab('checkin')} className={`flex-1 py-4 text-sm font-bold uppercase tracking-widest transition-colors ${activeTab === 'checkin' ? 'bg-white/10 text-white border-b-2 border-emerald-500' : 'text-slate-500 hover:text-slate-300'}`}>✅ Check-In</button>
            </div>
