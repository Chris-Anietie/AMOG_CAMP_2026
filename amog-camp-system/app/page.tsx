...
// INTRODUCING PROFESSIONAL ICONS
import { 
  Users, Coins, CreditCard, LogOut, Search, Plus, FileText, Download, 
  CheckCircle, AlertCircle, Home, User, Phone, Shirt
} from 'lucide-react';
...
export default function Home() {
  ...
  async function handleRecordPayment() {
    ...
    if (error) { showToast("Error: " + error.message, 'error'); } 
        ...
        // INSTANTLY UPDATE LOCAL STATE TO TRIGGER UI CHANGE IN MODAL
  ...
  return (
    <div className="min-h-screen font-sans text-gray-100 bg-[#0f172a] relative pb-20 overflow-x-hidden">
            ...
            return (
                <div key={p.id} className={`group relative bg-white/5 backdrop-blur-sm rounded-2xl p-5 border ${cardBorder} transition-all duration-300 hover:-translate-y-1 hover:bg-white/10 hover:shadow-2xl flex flex-col`}>
                  <div className="flex justify-between items-start mb-3">
                     <div><h2 className="text-lg font-bold text-white leading-tight truncate">{p.full_name}</h2><div className="flex items-center gap-2 mt-1 text-xs font-medium text-slate-400"><span className="bg-white/10 px-2 py-0.5 rounded uppercase tracking-wider flex items-center gap-1"><User className="w-3 h-3"/>{p.role}</span><span className="flex items-center gap-1 truncate"><Home className="w-3 h-3"/>{p.branch}</span></div></div>
                  ...
                  <button onClick={() => openModal(p)} className={`w-full py-3 rounded-xl font-bold text-sm shadow-lg transition-all flex items-center justify-center gap-2 
                    ${p.checked_in ? 'bg-white/10 text-slate-300 hover:bg-white/20' : 
                      (isOwing ? 'bg-amber-600 hover:bg-amber-500 text-white shadow-amber-900/30' : 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-900/30')}`}>
                     {p.checked_in ? <><FileText className="w-4 h-4"/> View Details</> : (isOwing ? <><CreditCard className="w-4 h-4"/> Manage Payment</> : <><CheckCircle className="w-4 h-4"/> Complete Check-In</>)}
                  </button>
                </div>
            );
          })}
      ...
      {/* UNIFIED ACTION MODAL - NO TABS, INTELLEGENT FLOW */}
      {selectedPerson && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-[2px] animate-in fade-in duration-200">
          <div className="bg-[#161f32] border border-white/10 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="bg-white/5 p-6 border-b border-white/10 flex justify-between items-center">
               <div><h2 className="text-xl font-bold text-white">{selectedPerson.full_name}</h2><span className="text-slate-400 text-xs font-bold uppercase tracking-wider">{selectedPerson.role} • {selectedPerson.branch}</span></div>
            ...
            <div className="p-6">
                {selectedPerson.checked_in ? (
                    // STATE 1: ALREADY CHECKED IN
                    <div className="text-center py-8">
                        <div className="w-16 h-16 bg-indigo-500/20 text-indigo-400 rounded-full flex items-center justify-center mx-auto mb-4"><CheckCircle className="w-10 h-10"/></div>
                        <h3 className="text-lg font-bold text-white">Already Checked In</h3>
                        <p className="text-slate-400 mt-2">Assigned to: <strong className="text-indigo-300">{selectedPerson.grace_school}</strong></p>
                        <p className="text-slate-500 text-sm mt-4 font-mono">Total Paid: ₵{selectedPerson.amount_paid}</p>
                    </div>
                ) : selectedPerson.amount_paid >= targetFee ? (
                    // STATE 2: FULLY PAID, READY TO CHECK IN
                    <div className="text-center space-y-6 py-4">
                        <div className="w-16 h-16 bg-emerald-500/20 text-emerald-400 rounded-full flex items-center justify-center mx-auto"><CheckCircle className="w-10 h-10"/></div>
                        <div><h3 className="text-xl font-bold text-white">Payment Cleared!</h3><p className="text-slate-400 mt-1">This person is ready for admittance.</p></div>
                        <button onClick={handleFinalCheckIn} disabled={processing} className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-lg font-bold shadow-xl shadow-emerald-900/30 flex items-center justify-center gap-2 transition-all active:scale-95">
                            {processing ? 'Processing...' : <><LogOut className="w-5 h-5 rotate-180"/> Admit To Camp</>}
                        </button>
                    </div>
                ) : (
                    // STATE 3: OWING, SHOW PAYMENT FORM
                    <div className="space-y-5">
                        <div className="bg-amber-500/10 border border-amber-500/30 p-3 rounded-xl flex justify-between items-center mb-4">
                            <span className="text-xs font-bold text-amber-400 uppercase tracking-wider">Balance Due</span>
                            <span className="text-lg font-mono font-bold text-amber-300">₵{targetFee - selectedPerson.amount_paid}</span>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1.5 block ml-1">Method</label><div className="relative"><CreditCard className="absolute left-3 top-3.5 w-4 h-4 text-slate-500"/><select className="w-full pl-9 p-3 rounded-xl bg-black/30 border border-white/10 text-white outline-none appearance-none" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)}><option className="bg-slate-900" value="Cash">Cash</option><option className="bg-slate-900" value="MoMo">MoMo</option></select></div></div>
                            <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1.5 block ml-1">Gender</label><div className="flex bg-black/30 rounded-xl p-1 border border-white/10"><button disabled={!!selectedPerson.gender} onClick={() => setGender('Male')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${gender === 'Male' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>Male</button><button disabled={!!selectedPerson.gender} onClick={() => setGender('Female')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${gender === 'Female' ? 'bg-pink-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>Female</button></div></div>
                        </div>
                        <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1.5 block ml-1">Amount Received</label><div className="relative"><span className="absolute left-4 top-3 text-lg font-bold text-slate-500">₵</span><input type="number" min="0" onKeyDown={(e) => ["-", "e", "+"].includes(e.key) && e.preventDefault()} className="w-full pl-10 p-3 rounded-xl bg-black/30 border border-white/10 text-2xl font-mono font-bold text-white outline-none focus:border-indigo-500/50 placeholder-slate-700" placeholder="0" value={topUpAmount} onChange={(e) => setTopUpAmount(e.target.value)} /></div></div>
                        <button onClick={handleRecordPayment} disabled={processing} className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-lg font-bold shadow-xl shadow-indigo-900/30 flex items-center justify-center gap-2 transition-all active:scale-95">
                            {processing ? 'Processing...' : <><Coins className="w-5 h-5"/> Record Payment</>}
                        </button>
                    </div>
                )}
            </div>
      ...
      {/* NEW REGISTRATION MODAL - CLEANER */}
      {isRegistering && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-[2px] animate-in fade-in duration-200">
